...
class PfGfUtilities {
	...
	public function setCustomerEmail( $form, $feed, $entry, array $varArray ): array {
		if ( $form["pagination"] === null ) {
			...
			if ( empty( $varArray['email_address'] ) ) {
				$emailFields = GFCommon::get_email_fields( $form );

				if ( ! empty( $emailFields ) ) {
					$varArray['email_address'] = $entry[$emailFields[0]->id];
				}
			}
		}
	 ...
	 * @return string
	 */
	public function setSecureStr( array $varArray, string $secureString ): string {
	...
	/**
	 * @param $discounts
	 * @param float $discount_amt
	 * @param string $query_string
	 *
	 * @return string
	 */
	public function lookForDiscounts( $discounts, float $discount_amt, string $query_string ): string {
